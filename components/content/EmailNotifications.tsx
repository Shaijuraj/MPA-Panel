    ...
    return (
        <div>
            ...
            <Card title="Email Templates">
                <p className="text-gray-500 mb-4">Enable or disable automated email notifications sent to users.</p>
                            ...
                            {templates.map(template => (
                                <tr key={template.id}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${template.status === 'Active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                                            {template.status}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <a href="#" className="text-primary hover:text-indigo-900 mr-4">Edit</a>
                                        <button onClick={() => toggleStatus(template.id)} className="text-sm font-semibold">
